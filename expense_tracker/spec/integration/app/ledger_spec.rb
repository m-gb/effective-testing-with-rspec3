...
module ExpenseTracker
  RSpec.describe Ledger, :aggregate_failures, :db do
    let(:ledger) { Ledger.new } # Initializer. Will be the same for each example.
    let(:expense) do
      {
        'payee'  => 'Starbucks',
        'amount' => 5.75,
    ...
    describe '#record' do
      context 'with a valid expense' do
        it 'successfully saves the expense in the DB' do
          result = ledger.record(expense) # Saves the expense.

          expect(result).to be_success
          # Reads the db and makes sure the expense really got saved.
    ...
    describe '#expenses_on' do
      it 'returns all expenses for the provided date' do
        # Records a few expenses into the ledger.
